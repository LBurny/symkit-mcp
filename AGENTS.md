...
- **Docs are emoji-free**: READMEs (EN/zh), CHANGELOG, ROADMAP, and everything under `docs/` / `formulas/` carry no emoji — not in headings, bullets, or prose. Plain typography (`→`, `✓`) is fine. Do not introduce new marks of any kind; the only pre-existing exceptions are the semantic ✅/❌ in the README comparison table and sentences that *describe* actual tool output (e.g. CHANGELOG's "legacy bools still render as ✅/❌") — preserve those verbatim when editing.
- **`docs/recommended-system-prompt.md` stays ≤80 lines**: the file IS the client system prompt (no title, preamble, or separator banners — it starts directly at `## Role and goal`). When editing, cut details a model can discover from tool output first; keep the sandbox-proven behavioral rules (parse traps like `E`/`I`/`f(x)`, verdict semantics, the tri-state dimension check, the `python_exec` contract).
