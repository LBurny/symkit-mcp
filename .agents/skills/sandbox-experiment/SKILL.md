...
description: symkit-mcp 黑箱沙箱试验全流程（十余轮实战沉淀，r16–r22 加量纲/Lean/现场报告专项）：搭建隔离 lab 实验台 → 写试验卡 → 无头 Claude 多 lane 运行（主控派 ≤3 个子代理驱动，主控只做划分/布置/检查/验收）→ 确定性探针复现 → 缺陷分析修复 → 验收闭环。只要用户提到沙箱试验/沙箱轮/黑箱测试/实验台/lab/试验卡/复现探针/验收，或想黑箱验证 symkit-mcp 某个版本或某批修复——即使没说"沙箱"二字（例如"跑一轮试试""搭个实验台""这版修好了再验一遍""复现这个缺陷"）——都使用本 skill。
---

# symkit-mcp 沙箱试验（黑箱轮）操作手册

黑箱原则：symkit-mcp 只通过它的 MCP 工具接口被评测。操作员（无头 Claude）和探针（stdio MCP client）从不读源码；源码侧的工作（根因、修复）由主控和修复代理在仓库里做。观察到的摩擦驱动框架改进——1.5.0→1.12.0 的十余轮实战（综合轮 + 量纲/Lean/算子面/python_exec 专项）都是这条闭环跑出来的。
...
- **lane 子代理**（ZCode 子代理，每 lane 一个）：驱动无头 Claude 跑 `run_suite.sh`（run_in_background + 轮询 stream.jsonl 进度），收齐 findings 后照录汇报。派工单写死：lane 编号、卡号区间、data-dir、RUN_PREFIX、max-turns 预算、"只跑卡不改卡不改 harness"。
- **操作员**（无头 Claude，被 lane 子代理驱动）：只拿试验卡黑箱操作，产物写 findings/。
- **修复代理**：单 Wave 内 TDD 实现，聚焦测试（`--no-cov -p no:cacheprovider`），越界需求停下报告。派工单里每个指控逐条对号其复现探针编号（r20 教训：不对号的指控会被代理自由发挥）。
...
13. **推荐系统提示词是被交付面，不是背景资料**。给真客户端的规范引导在仓库 `docs/recommended-system-prompt.md`（体例即提示词本身：无标题、无前言、无分隔线，**且 ≤80 行**——约束见 AGENTS.md）。跑卡时用 `SYSTEM_PROMPT_FILE=<lab 内副本绝对路径> bash run_suite.sh ...`，它经 `--append-system-prompt-file` 整份追加到 harness 默认提示之后——追加而非替换，替换会连带丢掉 harness 自己的工具说明。不设这个变量，操作员跑的是 harness 默认提示词，测不到交付给用户的引导。该文档里每条断言（`f(x)` 解析为未定义函数、`E`/`I` 保留为普通符号、`pi`/`oo` 才是常量、会话链与 `assume` 口径）都能被黑箱证伪：同一张卡注入与不注入各跑一次，两次都错是引擎缺陷，只有不注入时错则是提示词没交代清。
14. **探针入参逐字抄翻车现场**。从 stream.jsonl 里抠出操作员失败的确切字符串进探针，不要改写拼写——改写过的变体可能恰好绕开真漏洞（r22：探针写 `=` 变体全绿，而操作员现场的 `==` 拼写仍平板 failed；漏洞就藏在拼写差里）。**返回值字段名与形态同理照抄**（r23：F11 验收探针断言 `"step": {`，实际响应是标量 `"step": 1`，差点把已修好的项误判 FAIL）。
15. **探针 FAIL 先疑探针自己**。电池预期值历史出错率不低（r19 regress 首跑 4 FAIL 全是探针写错；r21 电池预期错 9 处）——FAIL 时先用独立路径（直调 sympy / 手算）核对预期值，再定缺陷清单。预期由主控亲写的纪律不变，但"亲写"不等于"写对"。
16. **"静默丢失"类指控要按时间线还原，归因与现象分开处置**。r23 task-11 报"assume 不进共享上下文"：前置探针（会话存活期）全绿 → 归因驳回；回原始 stream 按时间戳还原，真实现象在 `session_complete` **之后**（会话完成清场假设且无披露）。同一份指控可以是"归因错 + 现象真"，只按操作员的解释去复现会整条漏掉。
17. **"探针复现不出"只说明探针没覆盖到，不说明缺陷不存在**。硬规则 14 的补集：r23 的 G13 我第一次只抄了 `expression` 原句、漏了同一调用里的 `session: true`，三条探针全绿便写下"非引擎缺陷、report-only"；逐字重放**完整调用**（全部参数 + 拼写）后 6 形态里 3 形态复现（`det` 崩溃、`diff(x**3)` 静默二阶导、`eigenvals` 静默空）。判定"驳回/未复现"前，先把该 tool_use 的**整个 input 对象**抄进探针，逐字段核对；复现不出时先问"我漏了哪个参数"，别急着写 report-only。
18. **同族静默错比崩溃更该抓**。一个根因往往同时产出"崩溃 + 正确答案错"两种表现，崩溃会被操作员报上来，静默错不会——但静默错更危险。r23 G13 的根因是"同名算子被应用两次"（解析层已应用 + 请求层再应用），显性表现是 `det` 泄漏 TypeError，隐性表现是 `diff(x**3)→6*x`、`inv(inv(M))→M`、`eigenvals→()`。修一处崩溃时要顺手扫同根因的静默形态，并把它们一起写进测试与 CHANGELOG。
