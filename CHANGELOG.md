...
(`symkit-mcp-test-dim`: a 36-assertion deterministic battery, a 43-case
robustness battery that runs every case in its own process under a hard cap,
and 24 operator cards over three lanes). Sixteen defects were confirmed with
controls and fixed. The batteries are green (39/39 and 43/43, zero hangs, zero
crashes), the 24 cards ran SUCCESS with clean discipline, and the seven cards
whose findings drove a fix were re-run against the rebuilt wheel.

### Fixed

- A fractional exponent in a *unit string* is no longer laundered into
  "dimensionless". `int(power)` turned `length ** (1/3)` into `length ** 0`,
  which reads as dimensionless, so a symbol declared `sqrt(m)` was treated as
  dimensionless and `x**3` came back `consistent: true` with an empty dimension
  vector. A non-integer power is now reported as *unknown*, and an unresolvable
  unit is never reported as dimensionless either: `None` (undetermined) and `{}`
  (dimensionless) stay distinct everywhere.
- A dimensional inconsistency is no longer hidden by an unresolved symbol, and
  ...
  `null`, but any *definite* inconsistency further in is reported.
- Every unresolved symbol in a product is reported, not just the first one.
  `rho*v*t` with one declared unit used to name only `rho`.
- A partial sum no longer reports a determined dimension. `sqrt(m) + s` came
  back as `{time: 1}`, silently dropping the term that could not be reduced; a
  mismatch between the terms that *did* reduce is still reported.
- Unit strings a domain author actually writes now resolve: the SI micro prefix
  `µm`/`μm` (U+00B5 / U+03BC; `um` already worked, so the glyph spellings were
  the gap), `°C`/`℃`/`degC`/`°F`/`°`, and `%`/`‰`/`ppm` as dimensionless.
  Each of these used to degrade to "unknown", which turned the whole expression
  inconclusive instead of checked. The steradian is read as the dimensionless
  derived unit SI defines it to be, instead of leaking a `steradian` key into a
  vector whose contract says "keys are base quantities".
- Declared units survive a session reload, and `register_symbol` persists its
  declaration immediately. Units live in the symbol registry, which the session
  payload did not carry, and a declaration only reached disk when some *later*
  step happened to save the session — so a restart right after declaring units
  resumed a session that knew none of them, while `lookup_symbol` still answered
  with the *built-in catalogue* default (`rho` to `kg/m^3`) that the checker
  deliberately refuses to use. `session_resume` reported `success: true` and the
  steps were intact, so nothing revealed the loss.
...
- A recorded `dimension` step now carries its own verdict. Such a step *is* a
  ...
  finding the user had already been handed. It also contradicted itself: the
  post-check re-judged the archived expression against the *session's*
  declarations, which do not include units passed per call, so a step that had
  resolved temperature was annotated `dimension_unknown_symbols: ["x"]`.
  Inconsistent fails the step and the chain, consistent verifies, undetermined
  stays inconclusive.
- A recorded dimension step keeps an undetermined result. `result_dimension or
  ...
  "dimensionless" where the live response had said "no idea".
- `message` no longer appends "has net dimension: dimensionless" to an
  expression it has just called inconsistent (`exp(L)` read as one sentence
...
- A formula's per-variable units reach the verify chain.
  ...
  surface and the chain saw no units. (`session_load_formula` still takes a bare
  expression by signature, so it carries nothing — use
  `formula_get(..., load_into_session=True)`.)
...
- `unknown_symbols` names the symbols that need a declaration even when no units
  are given at all. Partial unit maps already named the missing ones; the
  all-missing case answered with an empty list, so the one response a caller
  reads to decide what to declare said nothing.
...
- A literal `0` on one side of an equation stays dimensionally polymorphic
  (`0 = m*v` is coherent; `5 = m*v` is not), and `-` stays both the "unknown
  unit" sentinel and the dimensionless marker.
...
reviewing the round-17 derivations.
